...
| 種別 | 対象 | ツール候補 |
| --- | --- | --- |
| 単体 | TypeScript service、計算ロジック | Vitest |
| API結合 | REST API | Vitest、Fastify inject |
| フロント | UI component | Vitest、Testing Library |
| E2E | 主要導線 | Playwright |
| AI評価 | OpenAI Responses APIのStructured Outputs検証、固定入力の出力検証 | Vitest、JSON Schema |
| インフラ | IaC構文、デプロイ後確認 | CDK synth、curl |
...
| `GET /api/auth/csrf` | CSRFトークンとCookieを取得できる |
| `GET /api/health` | DB接続が正常なら200 |
