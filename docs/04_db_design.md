...
- 運用データ: provider_fetch_logs、analysis_jobs
...
erDiagram
  ...
  analysis_jobs ||--o| analysis_reports : produces
```
...
| provider_payload | jsonb | null | 元レスポンス |
| created_at | timestamptz | not null | 作成日時 |
| updated_at | timestamptz | not null | 更新日時 |
...
| データ | 更新タイミング | 方針 |
| --- | --- | --- |
| 銘柄マスタ | 1日1回 | EventBridgeで夜間更新 |
| 株価 | 1日1回 | 日足のため営業日夜間更新 |
| 財務 | 1日1回 | 新規開示を確認 |
| AIレポート | ユーザー操作時 | 同一input_hashなら再利用可能 |

MVPではユーザーが銘柄詳細を開いたタイミングで不足データを取得するオンデマンド方式でもよいです。

<a id="derived-metrics"></a>
## 7. 派生指標

派生指標はAPIレスポンス用DTOで計算します。DBに保存するのは元データ中心とします。
