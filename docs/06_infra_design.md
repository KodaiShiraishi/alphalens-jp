...
MVPではWorkerを省略し、Go API内で同期的にAIレポート生成してもよいです。ただし、コード上は後からWorkerに切り出せるようにサービス層を分離します。
...
- MVPデモではNAT Gatewayを避け、外部通信が必要なECS TaskをPublic Subnetに置く選択もあり。
...
- MVP速度重視ならVercel
- AWSアピール重視ならAmplify Hosting
- 静的exportできる構成ならS3 + CloudFront
